        ...
        for fyrq in date_list:
            for cid, cinema_name in zip(cinema_id_list, cinema_name_list):
                for movieid, movie_name in zip(movie_id_list, movie_name_list):
                    ...
                    cinema = cinema_name.strip()
                    yield scrapy.Request(ajax_url, callback=lambda response, date=date, name=name, cinema=cinema: self.parse_ajax)

    def parse_ajax(self, response, date, name, cinema):
        screenings = []
        chooseOpi_body = response.xpath('//div[@class="chooseOpi_body"]/ul/li')
        for li in enumerate(chooseOpi_body):
        ...
        film = GewaraFilm(name=name, cinema=cinema, date=date, screenings=screenings)
        return film
